...
Hostess is a planned Second Life venue-manager aid focused on one narrow job: help a pre-provisioned account log in, choose its groups by display name, and send event notices with an existing landmark attachment to selected group targets.
...
- `hostess-core/` - reusable JVM core boundary for login, group listing, display-name targeting, existing-landmark attachment handling, and notice orchestration.
