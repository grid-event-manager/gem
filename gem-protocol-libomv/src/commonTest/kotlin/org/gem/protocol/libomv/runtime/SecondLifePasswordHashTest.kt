...
class SecondLifePasswordHashTest {
    ...
    fun `invalid secrets fail closed`() {
        val digestPort = testDigestPort()

        assertNull(SecondLifePasswordHash.fromSharedSecret("", digestPort))
        assertNull(SecondLifePasswordHash.fromSharedSecret("   ", digestPort))
        assertNull(SecondLifePasswordHash.fromSharedSecret("\$1\$not-md5", digestPort))
        assertNull(SecondLifePasswordHash.fromSharedSecret("12345678901234567", digestPort))
        assertNull(SecondLifePasswordHash.fromSharedSecret("secrét", digestPort))
    }

    private fun testDigestPort(): Md5DigestPort =
        Md5DigestPort { "8b2fee48cd255fddee9a662b55da4fd4" }
}
