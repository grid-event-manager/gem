...
class LibomvClientSession private constructor(
    ...
    private var activeSession: GemSession?,
    ...
    internal fun requireSession(session: GemSession): CoreFailure? {
        ...
        return when {
            ...
            !session.isActive -> CoreFailure(CoreFailureReason.LOGIN_FAILED, redactedMessage = "gem session inactive")
            active.sessionId != session.sessionId ->
                CoreFailure(CoreFailureReason.LOGIN_FAILED, redactedMessage = "gem session mismatch")
            else -> null
        }
    }

    internal fun requireIdentity(session: GemSession): LibomvSessionIdentityResult {
    ...
    internal fun inventoryRoots(session: GemSession): LoginInventoryRootsResult {
        val bindingFailure = requireSession(session)
        if (bindingFailure != null) {
    ...
    internal fun appearanceState(session: GemSession): LoginAppearanceStateResult {
        val bindingFailure = requireSession(session)
        if (bindingFailure != null) {
