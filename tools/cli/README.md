...
```bash
./gradlew :tools:cli:run --args="list-groups --mode fake --report build/reports/hostess/fake-list-groups.json"
./gradlew :tools:cli:run --args="send-notice --mode fake --target 'Venue Hosts' --subject 'Tonight' --body 'Doors at eight'"
./gradlew :tools:cli:run --args="live-proof --mode live --proof-scope full --report build/reports/hostess/live-full-cycle.json --account venue-proof --credential-env HOSTESS_PROOF_CREDENTIAL --existing-attachment-name 'Venue Landmark'"
```
