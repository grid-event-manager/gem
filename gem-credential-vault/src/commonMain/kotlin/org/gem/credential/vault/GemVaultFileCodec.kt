...
object GemVaultFileCodec {
    fun encode(plaintext: GemVaultPlaintext): ByteArray {
    ...
    fun decode(payloadBytes: ByteArray): GemVaultCodecResult {
        ...
        return validate(GemVaultPlaintext(profiles = profiles, credentials = credentials))
    }

    private fun validate(plaintext: GemVaultPlaintext): GemVaultCodecResult {
        ...
        plaintext.credentials.forEach { credential ->
            if (!credential.credentialHandle.isGemVaultCredentialHandle()) {
        ...
        return GemVaultCodecResult.Decoded(plaintext)
    }

    private fun corrupt(message: String? = "corrupt"): GemVaultCodecResult.Corrupt =
        GemVaultCodecResult.Corrupt(message)
}

sealed interface GemVaultCodecResult {
    data class Decoded(val plaintext: GemVaultPlaintext) : GemVaultCodecResult
    data class Corrupt(val message: String? = null) : GemVaultCodecResult
}

private class VaultPayloadWriter {
    private val bytes = mutableListOf<Byte>()

    fun writeProfile(profile: GemVaultProfileRecord) {
    ...
    fun writeCredential(credential: GemVaultCredentialRecord) {
...
private class VaultPayloadReader(
    ...
    fun readProfile(): GemVaultProfileRecord? {
        ...
        return GemVaultProfileRecord(profileId, loginName, label, credentialHandle, startLocation)
    }

    fun readCredential(): GemVaultCredentialRecord? {
        val credentialHandle = readCredentialHandle() ?: return null
        ...
        return GemVaultCredentialRecord(credentialHandle, loginUri, loginName, sharedSecret, startLocation)
    }
