...
class VaultAccessService(
    ...
    private val fileStore: GemVaultFileStore,
    ...
    fun open(): VaultAccessOpenResult {
        ...
        return VaultAccessOpenResult.Ready(
            credentialVault = EncryptedGemVault(session, credentialHandleRandom),
    ...
    fun reset(confirmed: Boolean): VaultAccessResetResult {
        ...
        return when (val vaultDelete = fileStore.delete()) {
            is GemVaultFileDeleteResult.StorageFailed -> VaultAccessResetResult.StorageFailed(vaultDelete.message)
...
internal class VaultDocumentSession private constructor(
    ...
    plaintext: GemVaultPlaintext,
) {
    private var keyMaterial: VaultKeyMaterial? = keyMaterial
    private var plaintext: GemVaultPlaintext? = plaintext
    private val deferredCredentialDeletes: MutableSet<CredentialHandle> = mutableSetOf()

    fun snapshot(): VaultDocumentSessionResult<GemVaultPlaintext> {
    ...
    fun replace(next: GemVaultPlaintext): VaultDocumentSessionResult<Unit> {
    ...
    fun pruneDeferredCredentialDeletes(
        profiles: List<GemVaultProfileRecord>,
        credentials: List<GemVaultCredentialRecord>,
    ): List<GemVaultCredentialRecord> {
    ...
    private fun persist(next: GemVaultPlaintext): VaultDocumentSessionResult<Unit> {
        val key = keyMaterial ?: return VaultDocumentSessionResult.KeySourceFailed("vault_locked")
        val payload = try {
            GemVaultFileCodec.encode(next)
        ...
        return when (val written = fileStore.writeAtomic(envelope)) {
            is GemVaultFileWriteResult.Written -> VaultDocumentSessionResult.Success(Unit)
            is GemVaultFileWriteResult.StorageFailed -> VaultDocumentSessionResult.StorageFailed(written.message)
        }
    }

    companion object {
        fun open(
            fileStore: GemVaultFileStore,
            cipher: JcaVaultCipher,
            keyMaterial: VaultKeyMaterial,
        ): VaultDocumentOpenResult =
            when (val read = fileStore.read()) {
                GemVaultFileReadResult.Missing -> createEmpty(fileStore, cipher, keyMaterial)
                is GemVaultFileReadResult.Read -> decryptExisting(fileStore, cipher, keyMaterial, read.bytes)
                is GemVaultFileReadResult.StorageFailed -> VaultDocumentOpenResult.StorageFailed(read.message)
            }
        ...
        private fun createEmpty(
            ...
            val empty = GemVaultPlaintext(profiles = emptyList(), credentials = emptyList())
        ...
        private fun decryptExisting(
            ...
            val plaintext = when (val decoded = GemVaultFileCodec.decode(payload)) {
                is GemVaultCodecResult.Decoded -> decoded.plaintext
                is GemVaultCodecResult.Corrupt -> return VaultDocumentOpenResult.CorruptVault(decoded.message)
            }
