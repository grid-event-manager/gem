...
data class SecondLifeTimeDisplay(
    ...
    fun label(textCatalogue: GemTextCatalogue): String {
        val meridiemText = when (meridiem) {
            SecondLifeMeridiem.AM -> textCatalogue.text(GemTextKey.MeridiemAm)
            SecondLifeMeridiem.PM -> textCatalogue.text(GemTextKey.MeridiemPm)
        }
        return "${textCatalogue.text(GemTextKey.SecondLifeTimePrefix)} $hour:${minute.twoDigits()} $meridiemText"
    }
}

object SecondLifeTimeFormatter {
    fun display(instant: GemInstant): SecondLifeTimeDisplay {
