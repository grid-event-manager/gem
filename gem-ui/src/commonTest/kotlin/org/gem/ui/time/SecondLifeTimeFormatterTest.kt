...
class SecondLifeTimeFormatterTest {
    ...
    fun `formats current second life time from pacific daylight time`() {
        val display = SecondLifeTimeFormatter.display(GemInstant(1_781_209_800_000L))

        assertEquals(SecondLifeTimeDisplay(hour = 1, minute = 30, meridiem = SecondLifeMeridiem.PM), display)
        assertEquals("SLT 1:30 PM", display.label(EnglishGemTextCatalogue))
    }

    @Test
    fun `applies spring daylight transition at second sunday in march`() {
        val before = SecondLifeTimeFormatter.display(GemInstant(1_772_963_940_000L))
        val after = SecondLifeTimeFormatter.display(GemInstant(1_772_964_000_000L))
    ...
    fun `applies autumn standard transition at first sunday in november`() {
        val before = SecondLifeTimeFormatter.display(GemInstant(1_793_523_540_000L))
        val after = SecondLifeTimeFormatter.display(GemInstant(1_793_523_600_000L))
